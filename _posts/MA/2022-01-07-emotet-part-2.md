...
# [Part 01](https://muha2xmad.github.io/malware-analysis/emotet-part-1/) summary
 ...
 found VirtualAlloc call in sub_417D50 its address 
...
- What is Packing:
	Malware authors use a number of tricks to avoid detection and analysis. One of the most popular methods is to employ a packer, 

- What is a packer: a tool that compresses, encrypts, and/or modifies a malicious file’s format.
	Or convert a single executable into many of executables

- Why using packers:  to anoid AV detection anti-analysis technique to make it harder for researchers to reverse engineer the code and
  obfuscate the payload Or  can also be used for legitimate ends, for example, to protect a program against cracking or copying


- We need to find the the original entry point (OEP).

- What is the OEP:  is the address of the malware's first instruction (where malicious code begins) before it was packed.
...
We need to know what is `VirtualAlloc`?
 If we research VirtualAlloc on Microsoft’s website, we see the full writeup on it: [Here](https://msdn.microsoft.com/en-us/library/windows/desktop/aa366887(v=vs.85).aspx)
...
So we put a breakpoint over this instruction `mov edx,dword ptr ds:[41C1B4]` by `f2` and press `f9` to hit the breakpoint.
...
And these functions do the same as above. So step over them `f8`. 
 Double click over a function to see what inside and press `-` button to get out without executing it. 
